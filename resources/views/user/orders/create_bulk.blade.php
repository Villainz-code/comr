...
<div class="checkout-page">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <nav class="flex items-center space-x-2 text-xs text-gray-500 mb-8">
            ...
            <span>›</span>
            <a href="{{ route('user.cart') }}" class="hover:text-white transition-colors">Keranjang</a>
            <span>›</span>
            <span class="text-gray-300">Checkout Bulk</span>
        </nav>

        <form method="POST" action="{{ route('user.order.storeBulk') }}" id="checkout-form">
            @csrf
            ...
            <div class="checkout-grid">
                ...
                <div class="checkout-right">
                    <div class="order-summary-sticky">
                        ...
                        {{-- Product Cards List --}}
                        <div class="mb-6 space-y-4 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                            @foreach($cartItems as $item)
                            <div class="order-product-card p-3 bg-[#111] border border-gray-800 rounded-xl relative">
                                <div class="product-thumb w-16 h-16 rounded bg-gray-800 flex items-center justify-center overflow-hidden shrink-0">
                                ...
                                <div class="product-details flex-1 ml-3">
                                    <h3 class="text-sm font-bold text-white truncate max-w-[200px]">{{ $item->product->name }}</h3>
                                    <p class="text-xs text-gray-500">{{ $item->product->category->name ?? 'Produk' }}</p>
                                    <div class="flex items-center gap-3 mt-1">
                                        <span class="text-xs text-gray-400">Jumlah: <span class="text-white">{{ $item->quantity }}</span></span>
                                        @if($item->selected_size)
                                        <span class="text-xs text-emerald-400">Ukuran: {{ $item->selected_size }}</span>
                                        @endif
                                    </div>
                                </div>
                                <div class="text-right ml-2 shrink-0">
                                    <p class="text-sm font-bold text-white whitespace-nowrap">Rp {{ number_format($item->product->price * $item->quantity, 0, ',', '.') }}</p>
                                </div>
                            </div>
                            @endforeach
                        </div>
                        ...
                        <div class="price-breakdown">
                            <div class="price-row">
                                <span class="price-label">Subtotal · <span id="summary-qty-text">{{ $totalQty }}</span> barang</span>
                                <span class="price-value" id="summary-subtotal" data-subtotal="{{ $subtotal }}">Rp {{ number_format($subtotal, 0, ',', '.') }}</span>
                            </div>
                            <div class="price-row">
                            ...
                            <div class="price-row">
                                ...
                                <span class="price-value" id="shipping-cost" data-item-count="{{ $cartItems->count() }}">Rp {{ number_format(15000 * $cartItems->count(), 0, ',', '.') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="total-section">
                            ...
                            <span class="total-value" id="summary-total">Rp {{ number_format($subtotal + (15000 * $cartItems->count()), 0, ',', '.') }}</span>
                        </div>
...
<script>
    ...
    function getSelectedShippingCost() {
        ...
        const perItemCost = selected ? (SHIPPING_COSTS[selected.value] || 15000) : SHIPPING_COSTS.regular;
        const itemCount = parseInt(shippingCostEl.dataset.itemCount) || 1;
        return perItemCost * itemCount;
    }

    function updateSummary() {
        const subtotal = parseInt(summarySubtotal.dataset.subtotal) || 0;
